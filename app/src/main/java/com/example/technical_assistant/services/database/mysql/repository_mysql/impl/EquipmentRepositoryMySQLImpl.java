...
import com.example.technical_assistant.services.database.mysql.repository_mysql.EquipmentRepositoryMySQL;
import com.example.technical_assistant.services.database.mysql.sintax.EquipmentTableMySQL;
import com.example.technical_assistant.services.database.mysql.sintax.impl.EquipmentTableMySQLImpl;
...
public class EquipmentRepositoryMySQLImpl implements EquipmentRepositoryMySQL {

    User user;
    ConnectMySQL connection;
    Language language = new LanguageImpl();
    @Override
    public Equipment getEquipmentById(User user, String companyName, Integer id) {
        this.user = user;
        ...
        try {
            connection.resultSetMySQL = connection.statementMySQL.executeQuery(equipmentTableMySQL.SELECT(this.user.getNameDB()));
            while (connection.resultSetMySQL.next()){
                if(connection.resultSetMySQL.getString("company").equals(companyName)
                && connection.resultSetMySQL.getInt("id") == id){
                    return getEquipment(connection.resultSetMySQL);
                }
            }
    ...
    @Override
    public List<Equipment> getEquipmentByCompanyName(User user, String companyName) {
        this.user = user;
        ...
        try {
            ...
            while (connection.resultSetMySQL.next()){
                if(connection.resultSetMySQL.getString("company").equals(companyName)){
                    equipmentList.add(getEquipment(connection.resultSetMySQL));
                }
            }
            connection.closeMySQL_DB();
        } catch (SQLException e) {
    ...
    @Override
    public List<Equipment> getEquipmentByRoomName(User user, String companyName, String roomName) {
        this.user = user;
        ...
        try {
            connection.resultSetMySQL = connection.statementMySQL.executeQuery(equipmentTableMySQL.SELECT(this.user.getNameDB()));
            while (connection.resultSetMySQL.next()){
                if(roomName.equals(connection.resultSetMySQL.getString("room")) &&
    ...
    @Override
    public void setEquipment(User user, Equipment equipment) {
        this.user = user;
        connection = new ConnectMySQL(this.user);
        connection.createEquipmentTableMySQL();
        EquipmentTableMySQL equipmentTableMySQL = new EquipmentTableMySQLImpl();
    ...
    @Override
    public void setListEquipment(User user, String companyName, List<Equipment> equipmentList) {
        this.user = user;
        connection = new ConnectMySQL(this.user);
    ...
    @Override
    public void updateEquipment(User user, String companyName, Equipment equipment) {
        this.user = user;
        connection = new ConnectMySQL(this.user);
            connection.createEquipmentTableMySQL();
            EquipmentTableMySQL equipmentTableMySQL = new EquipmentTableMySQLImpl();
            ...
            try {
                PreparedStatement cf = connection.connectionMySQL.prepareStatement(equipmentTableMySQL.UPDATE(this.user.getNameDB()));
                setEquipment(cf, equipment);
    ...
    @Override
    public void updateEquipmentList(User user, String companyName, List<Equipment> equipmentList) {
        this.user = user;
        connection = new ConnectMySQL(this.user);
        for (Equipment equipment : equipmentList){
            connection.createEquipmentTableMySQL();
